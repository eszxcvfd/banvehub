  ...
  try {
    ...
    if (!user) {
      return NextResponse.json(
        { error: 'UNAUTHORIZED', message: 'Yêu cầu đăng nhập để chuyển trạng thái xử lý rút tiền.' },
        { status: 401 },
      )
    }
    ...
    if (!user.roles?.includes('financeAdmin') && !user.roles?.includes('admin')) {
      return NextResponse.json(
        { error: 'FORBIDDEN', message: 'Chỉ Finance Admin hoặc Admin mới có quyền xử lý rút tiền.' },
        { status: 403 },
      )
    }
    ...
    const errorMsg = err instanceof Error ? err.message : 'Không thể chuyển sang trạng thái đang xử lý.'
    return NextResponse.json(
