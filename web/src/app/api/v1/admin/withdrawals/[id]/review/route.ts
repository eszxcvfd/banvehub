...
import { reviewWithdrawal } from '@/services/withdrawal'
  ...
  try {
    ...
    if (!user) {
      return NextResponse.json(
        { error: 'UNAUTHORIZED', message: 'Yêu cầu đăng nhập để thẩm định rút tiền.' },
        { status: 401 },
      )
    }
    ...
    const result = await reviewWithdrawal(payload, {
      withdrawalId,
      actorId: user.id,
    })
    ...
    const errorMsg = err instanceof Error ? err.message : 'Không thể tiếp nhận thẩm định.'
    return NextResponse.json(
