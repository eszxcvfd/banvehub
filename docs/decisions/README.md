...
| [0012 Refund policy](0012-refund-policy.md) | Accepted | Refunds are executed only by the operator (`financeAdmin`/`admin`) through the audited refund path — there is no automatic dispute-to-refund; requests arrive out of band through a contact channel the website must publish; eligibility is fault-based (seller or platform fault, never a buyer's change of mind) and the recorded reason must carry that basis; the buyer has 5 days from receipt to request a refund while the seller is paid 7 days after receipt, so an approved refund reverses a still-PENDING earning instead of paid money; the fault basis decides who bears it — a seller's fault reverses the seller's earning, the platform's fault refunds only the buyer, leaves that earning to mature and books no revenue on the order; no surface may show "Đã hoàn tiền" without an executed refund record, so a seller may not set that ticket resolution |
