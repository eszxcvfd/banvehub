...
| [0003 Treat the database as a config merge target](0003-database-config-merge-rebuild.md) | Accepted | Intended config lives in `daptin/schema/`; drifted persisted config is fixed by rebuilding the database, not by hand-editing `world_schema_json` |
